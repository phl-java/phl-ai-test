...
|------|------|------|
| id | Long | 主键，自增 |
| appName | String | 应用名称（取 initPrompt 前 12 位） |
| cover | String | 应用封面 URL |
| initPrompt | String | 初始化提示词 |
| codeGenType | String | 代码生成类型：html / multi_file / vue_project |
| deployKey | String | 部署标识（6 位随机字符串） |
| deployedTime | LocalDateTime | 部署时间 |
| priority | Integer | 优先级（99 = 精选应用） |
| userId | Long | 创建用户 ID |
| editTime | LocalDateTime | 编辑时间 |
| createTime | LocalDateTime | 创建时间 |
| updateTime | LocalDateTime | 更新时间 |
  ...
  → AppController.chatToGenCode(appId, message)
    → AppService.chatToGenCode()
      ...
      → 3. AiCodeGeneratorFacade.generateAndSaveCodeStream()
        → 根据 codeGenType 分发：
          ├── HTML / MULTI_FILE
          │   → AiCodeGeneratorService（流式调用 DeepSeek）
          │   → 收集完整代码 → 解析 → 保存文件
          └── VUE_PROJECT
...
| 模式 | 枚举值 | AI 模型 | 说明 |
|------|--------|---------|------|
| HTML 单文件 | `html` | deepseek-chat (streaming) | 生成单个 HTML 文件，JSON 格式返回 |
| 多文件 | `multi_file` | deepseek-chat (streaming) | 生成 HTML + CSS + JS 三个文件，JSON 格式返回 |
| Vue 工程 | `vue_project` | deepseek-reasoner (reasoning) | 生成完整 Vue3 + Vite 工程，通过工具调用逐文件写入 |
...
1. 克隆项目
2. 创建 MySQL 数据库 `yu_ai_code_mother`
3. 配置 `application-local.yml` 中的 DeepSeek API Key 和腾讯云 COS 密钥
4. 启动 Redis
5. 运行 `PhlAiCodeApplication`
6. 访问 API 文档：`http://localhost:8123/api/doc.html`