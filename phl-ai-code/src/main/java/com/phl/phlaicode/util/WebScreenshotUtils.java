...
public class WebScreenshotUtils {
    ...
    // 使用ThreadLocal为每个线程维护独立的WebDriver实例
    private static final ThreadLocal<WebDriver> webDriverThreadLocal = new ThreadLocal<>();

    // 用于跟踪所有已创建的WebDriver实例，以便在销毁时关闭所有实例
    private static final Map<Long, WebDriver> allWebDrivers = new ConcurrentHashMap<>();
    ...
    static {
        ...
        // 初始化主线程的WebDriver实例
        WebDriver mainDriver = initChromeDriver(DEFAULT_WIDTH, DEFAULT_HEIGHT);
        webDriverThreadLocal.set(mainDriver);
        allWebDrivers.put(Thread.currentThread().getId(), mainDriver);
    }
    ...
    public void destroy() {
        // 关闭所有线程中的WebDriver实例
        for (WebDriver driver : allWebDrivers.values()) {
            try {
                if (driver != null) {
                    driver.quit();
                }
            } catch (Exception e) {
                log.error("关闭WebDriver时出错", e);
            }
        }
        allWebDrivers.clear();
    ...
    private static WebDriver getCurrentWebDriver() {
        ...
        if (driver == null) {
            ...
            allWebDrivers.put(Thread.currentThread().getId(), driver);
        }
        return driver;
    }
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            ...
            // 获取当前线程的WebDriver实例
            WebDriver currentDriver = getCurrentWebDriver();
