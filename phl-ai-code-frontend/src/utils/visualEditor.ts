...
export class VisualEditor {
  ...
  /**
   * Apply a CSS style to a selected element in the iframe (real-time preview)
   */
  ...
  /**
   * Request computed styles for a selected element
   */
  ...
  /**
   * Reset all temporary style overrides for a selector
   */
  resetStyles(selector: string) {
    this.sendMessageToIframe({
      type: 'RESET_STYLES',
      data: { selector },
    })
  ...
  /**
   * Get fresh element info (e.g. after AI modifies code)
   */
  ...
  handleIframeMessage(event: MessageEvent) {
    ...
    switch (type) {
      ...
      case 'ELEMENT_HOVER':
        ...
        break
      case 'STYLE_APPLIED':
        console.log('Style applied:', data)
        break
      case 'COMPUTED_STYLES':
        console.log('Computed styles:', data)
        break
    }
  }
           ...
           switch (type) {
             ...
             case 'CLEAR_SELECTION':
              ...
              case 'CLEAR_ALL_EFFECTS':
                ...
                break;
              case 'APPLY_STYLE': {
                const { selector, property, value } = event.data.data;
                ...
                if (el) {
                  ...
                  try {
                    window.parent.postMessage({ type: 'STYLE_APPLIED', data: { selector, property, value } }, '*');
                  } catch(e) {}
                }
                break;
              }
              case 'GET_COMPUTED_STYLES': {
                const { selector: sel } = event.data.data;
                ...
                if (element) {
                  ...
                  try {
                    window.parent.postMessage({ type: 'COMPUTED_STYLES', data: { selector: sel, styles } }, '*');
                  } catch(e) {}
                }
                break;
              }
              case 'RESET_STYLES': {
                const { selector: rstSel } = event.data.data;
                const rstEl = document.querySelector(rstSel);
                if (rstEl) {
                  const originalStyle = rstEl.getAttribute('data-original-style');
                  rstEl.setAttribute('style', originalStyle || '');
                  rstEl.removeAttribute('data-original-style');
                }
                break;
              }
            }
